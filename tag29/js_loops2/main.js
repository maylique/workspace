...
// 2_2
console.log('%cneue aufgabe jonge', 'color: red');
...
const loopMe = () => {
    ...
    for (let i=0; i < input; i++){
        loopO.push('o')
        output.innerHTML = `L${loopO.join('')}p`
    }
...
const splitText = (text) => {
    let buffer = []
    let newText = Math.ceil(text.length / 100)

    for (let i = 0; i < newText; i++) {
        let start = i * 100
        let satz = text.substring(start, start + 100)
        buffer.push(`${satz} - ${i} of ${newText}`) 
    }
    return buffer
}
